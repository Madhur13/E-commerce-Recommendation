...
def fillDept(request):
    df = pd.read_csv('C:\Users\Madhur\Desktop\Instacart_Dataset\departments.csv')
...
def fillItem(request):
    df = pd.read_csv('C:\Users\Madhur\Desktop\Instacart_Dataset\products.csv')
    for row in df.itertuples():
        a = row[1]
        b = row[2]
        c = row[3]
...
def MLindex(request):
    ...
    if(not(bool(graph_G))):
        df=pd.read_csv('C:\Users\Madhur\Desktop\check.csv')
